...
//       console.log("Hitting:", `${import.meta.env.VITE_API_URL}/generate-kit`);

//       const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/generate-kit`, form);
...
const CampaignGenerator = () => {
  ...
  return (
    ...
    //       {/* Inputs */}
    //       <div className="grid grid-cols-2 gap-6 mb-4">
    //         <Input type="text" placeholder="Product Name" value={form.product_name} onChange={(e) => setForm({ ...form, product_name: e.target.value })} className="bg-purple-100 border-purple-300" />
    //         <Input type="text" placeholder="Product Category" value={form.product_category} onChange={(e) => setForm({ ...form, product_category: e.target.value })} className="bg-purple-100 border-purple-300" />
    //         <Input type="number" placeholder="Price (e.g. 999)" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="bg-purple-100 border-purple-300" />
    //         <Input type="search" placeholder="Festival (e.g. Diwali)" value={form.event_name} onChange={(e) => setForm({ ...form, event_name: e.target.value })} className="bg-purple-100 border-purple-300" />
    //         <Input type="search" placeholder="Location (e.g. Jaipur)" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="bg-purple-100 border-purple-300" />
    //       </div>
    ...
    //               <h4 className="text-xl font-bold mb-2">SHARE LOVE</h4>
    //               <img src={kit.generated_image_url} alt="WhatsApp Flyer" className="w-24 h-24 rounded-lg mx-auto mb-3 object-cover" />
    //               {kit.ad_copy && <p className="text-sm italic mt-2">{kit.ad_copy}</p>}
    //               <a href="https://www.meesho.com" target="_blank" rel="noopener noreferrer">
    //                 <Button className="bg-white text-purple-600 hover:bg-white/90 mt-3">Shop Now</Button>
    //               </a>
    //             </div>
    ...
    //   </div>
    // </div>
    // 👇 Only CSS/UX updated — no logic touched
...
<div className="min-h-screen bg-gradient-to-br from-white to-pink-50 font-sans">
  <div className="flex">
    ...
    <div className="flex-1 p-10">
      <h1 className="text-4xl font-bold text-fuchsia-700 mb-8">✨ Campaign Generator</h1>
